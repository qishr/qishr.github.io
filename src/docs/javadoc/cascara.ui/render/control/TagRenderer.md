Package [io.github.qishr.cascara.ui.render.control](index.md)

# Class TagRenderer
[java.lang.Object](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/lang/Object.html)<br/>
        [io.github.qishr.cascara.ui.render.AbstractRenderer](../AbstractRenderer.md)<br/>
                [io.github.qishr.cascara.ui.render.AbstractScalarRenderer](../AbstractScalarRenderer.md)<br/>
                        io.github.qishr.cascara.ui.render.control.TagRenderer<br/>
<br/>
All Implemented Interfaces:<br/>
    [ScalarRenderer](../../api/render/ScalarRenderer.md)


----

<span style="font-family: monospace; font-size: 80%;">public class __TagRenderer__<br/>extends [AbstractScalarRenderer](../AbstractScalarRenderer.md)
</span>


## Constructor Summary

| Constructor   | Description |
|---------------|-------------|
| TagRenderer() |             |



## Method Summary

| Modifier and Type                                                                                                    | Method                                                                                                                                                                                                                                                                                                                                                                 | Description |
|----------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-------------|
| public [Node](https://docs.oracle.com/en/java/java-components/javafx/25/docs/javafx.graphics/javafx/scene/Node.html) | [render](#render)([Labeled](https://docs.oracle.com/en/java/java-components/javafx/25/docs/javafx.controls/javafx/scene/control/Labeled.html) cell, [Object](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/lang/Object.html) data, [DataProvider](../../api/data/DataProvider.md) dataProvider, [FieldMetadata](../../form/FieldMetadata.md) meta) |             |
| public static [String](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/lang/String.html)           | [getContrastColor](#getcontrastcolor)([String](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/lang/String.html) colorStr)                                                                                                                                                                                                                           |             |


### Methods inherited from [AbstractRenderer](../AbstractRenderer.md)

getSchemaType, getSchemaFormat, getContentType, getServiceProperties

### Methods inherited from [AbstractScalarRenderer](../AbstractScalarRenderer.md)

extractString, extractString


## Method Details

### render

<span style="font-family: monospace; font-size: 80%;">public [Node](https://docs.oracle.com/en/java/java-components/javafx/25/docs/javafx.graphics/javafx/scene/Node.html) __render__([Labeled](https://docs.oracle.com/en/java/java-components/javafx/25/docs/javafx.controls/javafx/scene/control/Labeled.html) cell, [Object](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/lang/Object.html) data, [DataProvider](../../api/data/DataProvider.md) dataProvider, [FieldMetadata](../../form/FieldMetadata.md) meta)</span>



**Specified By:**

[ScalarRenderer](../../api/render/ScalarRenderer.md)


---

### getContrastColor

<span style="font-family: monospace; font-size: 80%;">public static [String](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/lang/String.html) __getContrastColor__([String](https://docs.oracle.com/en/java/javase/24/docs/api/java.base/java/lang/String.html) colorStr)</span>




---

